  ...
  return (
    <div className='bg-bg-color px-8 py-4 rounded-md flex justify-between items-center gap-6'>
      ...
      <div className='space-y-4'>
        <Link to={`/coffees/${_id}`} className='bg-[#D2B48C] text-white w-12 h-12 flex justify-center items-center text-xl rounded-md'><AiFillEye /></Link>
        <Link to={`/update/${_id}`} className='bg-[#3C393B] text-white w-12 h-12 flex justify-center items-center text-xl rounded-md'><BiSolidPencil /></Link>
        <button className='bg-[#EA4744] text-white w-12 h-12 flex justify-center items-center text-xl rounded-md' onClick={() => handleDelete(_id)}><MdDelete /></button>
      </div>
    </div>
  );
};
